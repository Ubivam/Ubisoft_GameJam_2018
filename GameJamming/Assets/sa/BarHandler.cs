...
public class BarHandler : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int i = 0; i < BAR_NUM; ++i) {
            ...
            float colorFactor = BAR_NUM / (i + 0.1f);
            barArray[i].GetComponentInChildren<Renderer>().material.color = new Color(
                firstCol.r * (1 - colorFactor) + secondCol.r * colorFactor,
                secondCol.g * (1 - colorFactor) + firstCol.g * colorFactor, 1);
    ...
    void Update()
    {
       ...
       if (Input.GetKeyDown(KeyCode.Alpha1))
        {
            asd = true;
            scaleIndex = 0 * (BAR_NUM / 10);
        }
        else if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            scaleIndex = 1 * (BAR_NUM / 10);
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha3))
        {
            scaleIndex = 2 * (BAR_NUM / 10);
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha4))
        {
            scaleIndex = 3 * (BAR_NUM / 10);
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha5))
        {
            scaleIndex = 4 * (BAR_NUM / 10);
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha6))
        {
            scaleIndex = 5 * (BAR_NUM / 10);
        }
        else if (Input.GetKeyDown(KeyCode.Alpha7))
        {
            scaleIndex = 6 * (BAR_NUM / 10);;
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha8))
        {
            scaleIndex = 7 * (BAR_NUM / 10);
            asd = true;
        }
        else if (Input.GetKeyDown(KeyCode.Alpha9))
        {
            scaleIndex = 8 * (BAR_NUM / 10);
            asd = true;
        }
    ...
    private void FixedUpdate()
    {
        for (int i = 0; i < BAR_NUM; ++i)
        {
            ...
            //factor = Mathf.Sin(peekMul * distance) / ( distance);

            //if (barArray[i].transform.position.y + 0.1 >= initPos[i].y + factor)
            //    factor = 0;

          
            //r[i].MovePosition(Vector3.Lerp(initPos[i], initPos[i] + Vector3.up * factor*Time.deltaTime, Time.fixedDeltaTime));
